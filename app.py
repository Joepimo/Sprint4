...
from flask import Flask, render_template, jsonify, request, url_for, g, session
...
def login():
    if request.method == "GET":
        ...
        return render_template('login.html',form=formulario, mensaje="Usuario o contraseña no válido.")
...
@app.route("/mensajes/listado/json",methods=["GET","POST"])
@login_required
def get_listado_mensajes_json():
    resultados = mensaje.listado()
    
    return jsonify({ "Result": "OK", "Records":resultados, "TotalRecordCount":len(resultados) })
